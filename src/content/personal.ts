 ...
 *
 * He wrote these twelve lines and they are set as he wrote them, with one
 * character changed: a full stop after "Thrust SSC", because every other line
 * carries one. The Recurly line originally ran a dash into its second clause;
 * he asked for the dash gone, so that clause is its own sentence. Nothing else
 * is edited, and nothing is added -- if a fact is missing from this list it is
 * missing because he did not put it there.
 *
...
export const story: readonly string[] = [
  ...
  "Worked for a San Francisco-based YC startup, QualGent (YC X25), reporting to the CTO, and built agent infrastructure into production for the first time.",
