...
LOCAL = 1.4      # local contrast strength (unsharp at 4 cells)
GAMMA = 1.1
CONTRAST = 1.5   # S-curve about the midtone
...
def sample(win: Image.Image, cols: int, rows: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    if LOCAL > 0:
        wide = Image.fromarray((lum * 255).astype(np.uint8)).filter(
            ImageFilter.GaussianBlur(radius=pitch * 4)
        )
    ...
    eroded = np.asarray(amask.filter(ImageFilter.MinFilter(int(pitch * 1.5) | 1))).astype(np.float32) / 255.0
...
def main() -> None:
    ...
    f96, w96 = sample(main_win, 96, 120)
    f64, w64 = sample(main_win, 64, 80)
    fab, wab = sample(about_win, 64, 80)
    f48, w48 = sample(main_win, 48, 60)

    # The name passes over the board's top-left corner at >= 80rem: it must be sky.
    assert not f96[:24, :24].any(), "top-left 24x24 cells of the 96 field must be empty (move MAIN_CROP)"
    assert not f64[:16, :16].any(), "top-left 16x16 cells of the 64 field must be empty"

    d96 = find_datum(f96, to_cell(DATUM_HINT, MAIN_CROP, 96))
    d64 = find_datum(f64, to_cell(DATUM_HINT, MAIN_CROP, 64))
    dab = find_datum(fab, to_cell(DATUM_HINT, ABOUT_CROP, 64))
    d48 = find_datum(f48, to_cell(DATUM_HINT, MAIN_CROP, 48))
    c96 = to_cell(CENTER_HINT, MAIN_CROP, 96)
    c64 = to_cell(CENTER_HINT, MAIN_CROP, 64)
    cab = to_cell(CENTER_HINT, ABOUT_CROP, 64)

    r96, r64, rab, r48 = rim_indices(f96, w96), rim_indices(f64, w64), rim_indices(fab, wab), rim_indices(f48, w48)

    (CONTENT / "portrait-field-96.ts").write_text(ts_module("portraitField96", f96, d96, c96, r96, "Hero at >= 48rem. Window MAIN_CROP of the photograph."))
    (CONTENT / "portrait-field-64.ts").write_text(ts_module("portraitField64", f64, d64, c64, r64, "Hero below 48rem and the Contact afterimage. Same window, resampled."))
    (CONTENT / "portrait-field-about.ts").write_text(ts_module("portraitFieldAbout", fab, dab, cab, rab, "About: the face only, second angle. Window ABOUT_CROP."))
    ...
    render(f96, d96, set(r96), 12, True).save(PUBLIC / "dots-96@2x.webp", quality=90, method=6)
    render(f64, d64, set(r64), 10, True).save(PUBLIC / "dots-64@2x.webp", quality=90, method=6)
    render(fab, dab, set(rab), 10, True).save(PUBLIC / "dots-about@2x.webp", quality=90, method=6)
    ...
    if args.preview:
        ...
        render(f96, d96, set(r96), 7, False).save(out / "bake-96.png")
        render(f64, d64, set(r64), 9, False).save(out / "bake-64.png")
        render(fab, dab, set(rab), 9, False).save(out / "bake-about.png")
